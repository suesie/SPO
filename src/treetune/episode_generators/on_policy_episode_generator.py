...
from treetune.question_samplers import QuestionSampler
...
class OnPolicyEpisodeGenerator(EpisodeGenerator):
    ...
    def __init__(
        ...
        question_sampler: Lazy[QuestionSampler],
        ...
        **kwargs,
        ...
        # self.question_sampler_dir = self.exp_root / "question_sampler"
        # self.question_sampler_dir.mkdir(parents=True, exist_ok=True)
            ...
            if self.total_num_iterations is None:
                ...
                if num_repeats > 1:
                    ...
                    if self.distributed_state.is_main_process:
                        ...
                        new_ds.save_to_disk(str(self.temp_dir_root / "cached_dataset"))
                        del new_ds
                        release_memory()
    ...
    def generate(
        ...
        this_process_device_index = this_process_device.index + self.gpu_offset
        release_memory()

        if self.vllm_min_available_gpu_memory_mb is not None:
            total_mem_mb = (
                torch.cuda.get_device_properties(0).total_memory # We assume all the gpus on the machine is the same, so we can simply use gpu_0 to get total memory
                / 1024**2
            )
            ...
            logger.info(
                ...
                f"Waiting for GPU{this_process_device_index} used memory to be below {used_threshold_mb} MB. "
                f"Total GPU memory: {total_mem_mb} MB."
            )
            wait_for_memory_release(
                this_process_device_index,
                threshold_mb=used_threshold_mb,
            )

        gpu_memory_usage_before_mb = get_gpu_memory()[this_process_device_index]
        ...
        if self._orig_ds is None:
            ...
            # question_sampler_dir = self.question_sampler_dir / f"iteration__{iteration - 1:04d}"
            # if os.path.exists(question_sampler_dir):
            #     logger.info("Load question sampler from disk.")
            #     self.question_sampler = QuestionSampler.load_from_disk(question_sampler_dir) # Resume the question sampler from the last iteration
            # else:
            #     logger.info("Initialize question sampler.")
            #     self.question_sampler = self.question_sampler.construct(
            #         total_num_questions=len(self._orig_ds)
            #     )

        dataset = self._orig_ds
        # dataset = dataset.select(range(100)) # TODO: Debug on the small dataset
        if self.dataset_num_samples_per_iteration is not None:
        ...
        # selected_questions = self.question_sampler.select_question(num_samples)
        # dataset = dataset.select(selected_questions)

        # logger.info(f"Selected questions on {process_index}: {selected_questions}")
        ...
        vllm_init_fn = self._get_vllm_init_fn(
            ...
            process_index=process_index + self.gpu_offset,
            seed=seed,
            iteration=iteration,
        )
        ...
        # assert len(episodes_ds_shard) > 0, "No episodes generated."
        if len(episodes_lst) > 0:
        ...
        self._cloud_log({**metrics, "train/global_iteration": iteration})
        ...
        if self.is_main_process():
            ...
            assert len(shard_paths) > 0, "This should never happened !"
        ...
        self.distributed_state.wait_for_everyone()
        episodes = Dataset.load_from_disk(str(temp_dir / "episodes" / "merged")) # Here all the processes have the whole episodes dataset
        ...
        # question_sampler_stats = self.question_sampler.update_question_metrics(episodes)
        # question_sampler_metric = {f"question_sampler/{k}": v for k, v in question_sampler_stats.items()}

        # if len(question_sampler_metric) > 0:
        #     self._cloud_log({**question_sampler_metric, "train/global_iteration": iteration})

        # if self.is_main_process():
        #     QuestionSampler.save_to_disk(self.question_sampler, self.question_sampler_dir / f"iteration__{iteration:04d}")
    ...
    def _get_vllm_init_fn(
        ...
        if vllm_gpu_memory_utilization == "auto":
            ...
            # remaining_mem_mb = min(remaining_mem_mb, 16384) # TODO: We need to init actor engine during the vllm generation, so for the first time, we can't let vllm use too much memory
            vllm_gpu_memory_utilization = round(remaining_mem_mb / total_mem_mb, 2)
            # vllm_gpu_memory_utilization = min(vllm_gpu_memory_utilization, 0.5)
